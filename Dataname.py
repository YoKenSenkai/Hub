...
def showdata():
    ...
    sql = ("SELECT item_list.id, item_list.id_item, item_list.brand, item_list.serial_nb, customer.fname, customer.lname, dm.department FROM item_list INNER JOIN customer ON item_list.fname = customer.fname INNER JOIN dm ON customer.department = dm.department")
    sql2 = ("SELECT * FROM dm")
    cursor.execute(sql)
